...
type Parser struct {
	...
	// Format: 2025/12/06 00:00:14.136976 from 95.25.117.121:10976 accepted udp:142.250.130.106:443 [VTR-PL >> DIRECT] email: us_demo
...
func New() *Parser {
	...
	// from ([\w.:]+):(\d+) - source IP:port (supports IPv6)
	// accepted (tcp|udp):(.+?) - protocol and destination
	// \[(.+?) >> (.+?)\] - inbound >> outbound
	// email: (\S+) - user email
	regex := regexp.MustCompile(
		`^(\d{4}/\d{2}/\d{2} \d{2}:\d{2}:\d{2}\.\d+)\s+from\s+([\w\[\]:.-]+):(\d+)\s+accepted\s+(tcp|udp):(.+?)\s+\[(.+?)\s+>>\s+(.+?)\]\s+email:\s+(\S+)`,
	)
